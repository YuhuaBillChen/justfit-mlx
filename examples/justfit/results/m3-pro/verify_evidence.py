...
NAMES = ["fixed-32k-6k-r1", "fixed-32k-6k-r2", "fixed-32k-6k-r3",
         "b1-cold-240k-16k", "b2-cold-128k-16k",
         "b4-incumbent-128k-3x8k-12k"]
SHAPES = [([32768], 6144)] * 3 + [([245760], 16384),
          ([131072, 131072], 16384), ([131072, 8192, 8192, 8192], 12288)]
...
def audit(root):
    ...
    for name, (prompts, output) in zip(NAMES, SHAPES):
        ...
        require(sorted(c["requested_prompt_tokens"] for c in clients) == sorted(prompts), name)
        ...
        for client in clients:
            ...
            require(usage["prompt_tokens"] == client["requested_prompt_tokens"] and
                    usage["completion_tokens"] == output and
                    usage["total_tokens"] == client["requested_prompt_tokens"] + output and
                    usage["prompt_tokens_details"]["cached_tokens"] == 0, name + ": token totals")
            digest = hashlib.sha256("".join(text).encode()).hexdigest()
            require(digest == client["text_sha256"] == lane["text_sha256"], name + ": text hash")
            ...
            require(len(start) == 1 and start[0]["prompt_tokens"] == client["requested_prompt_tokens"], name)
            ...
            require(total == output and seq == sorted(seq), name + ": decode count/timing")
        ...
        count = output - 1 if len(points) == 1 else sum(begin < t < end for p in points for t in p)
        peak = max(s["phys_footprint_bytes"] for s in samples) / 2**20
        require(peak < 21000, name + ": sampled guard")
        pool = re.findall(r"high_water_pages=(\d+) capacity_pages=(\d+)", (folder / "server.log").read_text())
        ...
        calculated = {
            ...
            "wall_seconds": max(c["ended_at"] for c in clients) - min(c["started_at"] for c in clients),
        }
        for key, value in calculated.items():
            require(math.isclose(value, metrics[key], rel_tol=1e-10, abs_tol=1e-8), name + ": " + key)
        require(len(set(token_hashes)) == len(set(text_hashes)) == 1, name + ": lane output agreement")
        rows.append(dict(name=name, **calculated, token_ids_sha256=token_hashes[0], text_sha256=text_hashes[0]))
        for file in sorted(folder.iterdir()):
            if file.name in {"metrics.json", "clients.json", "events.jsonl", "footprint.jsonl", "server.log"} or file.name.startswith("lane-") and file.suffix == ".sse":
    ...
    require(len({r["token_ids_sha256"] for r in rows[:3]}) == 1, "Fixed repeat hash mismatch")
    require(rows[3]["token_ids_sha256"] == rows[4]["token_ids_sha256"], "B1/B2 hash mismatch")
    return {"status": "passed", "scope": "Independent raw SSE, token-event, timing, sampled footprint and pool audit; no model rerun or weight rehash",
            "rows": rows, "fixed_n3_medians": {k: statistics.median(r[k] for r in rows[:3]) for k in
                ("prefill_work_tps", "decode_tps", "sampled_peak_phys_footprint_mib")},
