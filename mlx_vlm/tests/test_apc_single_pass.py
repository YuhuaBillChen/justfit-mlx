...
def test_single_pass_matches_mlx_for_noncontiguous_dtypes(tmp_path):
    arrays = {str(i): mx.arange(48).astype(dtype).reshape(4, 12)[:, ::3]
              for i, dtype in enumerate([mx.float16, mx.bfloat16, mx.float32,
                                         mx.int32, mx.uint32, mx.uint8, mx.bool_])}
...
def test_sink_forwards_exactly_one_payload_and_rejects_truncation():
    ...
    encoded = io.BytesIO(); mx.save_safetensors(encoded, {'a': array})
    payload = encoded.getvalue()
    output = io.BytesIO()
    sink = PayloadSink(output, {'a': dict(dtype='U32', shape=[16], data_offsets=[0,64])})
