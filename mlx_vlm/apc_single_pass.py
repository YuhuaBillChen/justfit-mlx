...
class PayloadSink:
    ...
    def write(self, data):
        ...
        while len(view) and not self.validated:
            need = (8 if self.header_size is None else 8 + self.header_size) - len(
                self.header
            )
            ...
            if (
                ...
                and len(self.header) == 8 + self.header_size
            ):
                actual = json.loads(self.header[8:])
